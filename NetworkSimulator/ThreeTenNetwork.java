...
class ThreeTenNetwork {
	 ...
	 * @param graph is graph to use.
	 * @param startNode is node to start with
	 * @param queue is use to hold all nodes from graph
	 * @param hostToDijkstraNode is host to get to dijkstra node.
	 */
	public static void setupDijkstras(Network graph, Host startNode, PriorityQueue<DijkstraNode> queue, HashTable<Host,DijkstraNode> hostToDijkstraNode) {
		...
		int length = graph.verticesCollection.size();
		for (int i = 0; i < length; i++) {
			Host temp = graph.verticesCollection.get(i);
			current=setDistanceStartNode(startNode,temp);
			queue.add(current);
			hostToDijkstraNode.add(current.host,current);
		}
	 ...
	 * @param startNode is starting node
	 * @return node to add
	 */
	private static DijkstraNode setDistanceStartNode( Host startNode,Host temp) {
		DijkstraNode node = new DijkstraNode(temp);
		if(node.host == startNode)
			node.distance = 0;
		return node;
	}
	...
	/**
	 * Set Routing Table for start node.
	 * @param graph is the graph to use.
	 * @param startNode is Host to started with.
	 * @param hostToDijkstraNode is host to get to dijkstra node.
	 */
	public static void setupTables(Network graph, Host startNode, HashTable<Host,DijkstraNode> hostToDijkstraNode) {
		...
		//YOUR CODE HERE
		DijkstraNode other,hop;
		RoutingTable tableForStartNode =  startNode.getRoutingTable();
		int length = graph.verticesCollection.size();
		for (int i = 0; i <length ; i++) {
			Host temp = graph.verticesCollection.get(i);
			other = hostToDijkstraNode.get(temp);
			hop = other;
			addPairToTable(other,hop,startNode,tableForStartNode);
		}
	...
	/**
	 * Add the pair (other host, next hop) to the routing table for the start node.
	 * @param other is other hose
	 * @param hop is next hope
	 * @param startNode is start node
	 * @param table is the routing table
	 */
	private static void addPairToTable(DijkstraNode other, DijkstraNode hop,Host startNode, RoutingTable table) {
		while(hop.parent!=null){
			if(hop.parent.host==startNode){
				table.add(other.host,hop.host);
				break;
			}
			else {
				hop = hop.parent;
