...
public class Network implements Graph<Host,Connection>, DirectedGraph<Host,Connection> {
	 ...
	 * Vertices holder.
	 */
	LinkedList<Host> verticesCollection = new LinkedList<>();
	/**
	 * Edges holder.
	 */
	LinkedList<Connection> edgesCollection = new LinkedList<>();
	...
	public boolean containsVertex(Host vertex) {
		return verticesCollection.contains(vertex);
	}
	...
	public boolean containsEdge(Connection edge) {
		...
		return edgesCollection.contains(edge);
	}
	...
	public int getVertexCount() {
		return verticesCollection.size();
	}
	...
	public Collection<Connection> getOutEdges(Host vertex) {
		...
		ArrayOfListsOfPairs.Node<Host, Connection> current = storage.headPointer(index);
		LinkedList<Connection> outEdgesList = new LinkedList<>();
		while(current != null){
			outEdgesList.add(current.pair.getValue());
	...
	public Collection<Connection> getInEdges(Host vertex) {
		LinkedList<Connection> inEdges = new LinkedList<>();
		for(int i =0; i<storage.getNumLists();i++){
			Connection c = storage.getEdge(vertex,i);
			if(c != null){
				inEdges.add(c);}
		}
		return inEdges;
	...
	public Collection<Host> getPredecessors(Host vertex) {
		LinkedList<Host> predecessors = new LinkedList<>();
		for(int i=0; i<storage.getNumLists(); i++) {
			if(storage.findPredecessor(vertex, i)){
			  	predecessors.add(verticesCollection.get(i));
			}
		}
		return predecessors;
	}
	...
	public Collection<Host> getSuccessors(Host vertex) {
		int index=getIndex(vertex);
		LinkedList<Host> successors = new LinkedList<>();
		ArrayOfListsOfPairs.Node<Host, Connection> current = storage.headPointer(index);
		while(current != null){
			successors.add(current.pair.getKey());
			current=current.next;
		}
		return successors;
	}
	...
	public Host getSource(Connection directedEdge) {
		...
		int sourceLocation = storage.findSource(directedEdge);

		for(int i = 0; i<verticesCollection.size();i++){
			Host temp = verticesCollection.get(i);
			if(getIndex(temp) == sourceLocation){
				return temp;
			}
		}
	...
	public Host getDest(Connection directedEdge) {
		...
		Host temp = storage.findDestHost(directedEdge);
		return temp;
	...
	public Connection findEdge(Host v1, Host v2) {
		if(!containsVertex(v1) || !containsVertex(v2) || !connectedCheck(v1,v2)){ return null; }
		int index=getIndex(v1);
		return storage.getEdge(v2,index);
	}
	...
	public boolean addEdge(Connection e, Host v1, Host v2) {
		if(!containsVertex(v1) || !containsVertex(v2)){
			throw new IllegalArgumentException("This graph does not contain v1, v2");
		}
		int index = getIndex(v1);
		KeyValuePair<Host,Connection> pair = new KeyValuePair<>(v2,e);
		storage.add(pair,index);
		edgesCollection.add(e);
		return true;
	}
	...
	public boolean addVertex(Host vertex) {
		if(!containsVertex(vertex)){
			...
			throw new IllegalArgumentException("Invalid Null Vertex");
		}
		return false;
	}
	...
	public boolean removeVertex(Host vertex) {
		Collection<Connection> edgesToRemove = new LinkedList<>();
		if(vertex == null || !containsVertex(vertex)){
			return false;
		}
		else{
			edgesCollection.removeAll(getInEdges(vertex));
			edgesCollection.removeAll(getOutEdges(vertex));
			verticesCollection.remove(vertex);
			return true;
		}
	}
	...
	/**
	 * Get index of host in array.
	 * @param v1 host
	 * @return index
	 */
	private int getIndex(Host v1){
		return v1.getId();
	}

	/**
	 * Check to see if v1 is connected to v2.
	 * @param v1 host 1
	 * @param v2 host 2
	 * @return true if connected else return false
	 */
	private boolean connectedCheck (Host v1, Host v2){
		int index = getIndex(v1);
		return storage.containsKey(v2,index);
	}
	...
	public static void main(String[] args) {
		...
		graph.addEdge(connections[2],hosts[0],hosts[3]);
		graph.addEdge(connections[3],hosts[3],hosts[1]);
		graph.addEdge(connections[4],hosts[4],hosts[1]);
	 ...
	 * <>Host<> the vertex type for the graph factory
	 * <>Connection<> the edge type for the graph factory
	 * @return graph
	 */
	@SuppressWarnings("unchecked")
